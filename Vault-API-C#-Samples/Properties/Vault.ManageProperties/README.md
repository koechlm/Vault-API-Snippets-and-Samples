# Vault.ManageProperties

Helper class to manage Autodesk Vault file properties including updates and synchronization using the filestore service.

## Prerequisites

- **Autodesk Vault 2026 SDK** must be installed (assemblies are referenced from the SDK installation path)
- **.NET Framework 4.8**
- **x64 platform target**

## Installation

First, add the GitHub Packages source (one-time setup):

```
dotnet nuget add source "https://nuget.pkg.github.com/koechlm/index.json" --name "github-koechlm" --username YOUR_GITHUB_USERNAME --password YOUR_GITHUB_PAT --store-password-in-clear-text
```

Then install the package:

```
dotnet add package Vault.ManageProperties --source "github-koechlm"
```

Or via Package Manager Console:

```
Install-Package Vault.ManageProperties -Source "github-koechlm"
```

> **Important:** After installing, ensure your project references the following Vault SDK assemblies from your local SDK installation (`%ProgramW6432%\Autodesk\Autodesk Vault 2026 SDK\bin\x64\`):
