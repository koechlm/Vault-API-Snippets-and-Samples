    ...
    public class ManageProperties
    {
            ...
            out string[] cloakedEntityClasses, bool force = false, Dictionary<string, string> overridePropValues = null)
        {
            ...
            try // if anything goes wrong from here on out, undo the checkout
            {
                ...
                {
                    string sysName = filePropDispToSysNames.TryGetValue(name, out string foundSysName) ? foundSysName : name;
                    PropertyDefinition propDef = connection.PropertyManager.GetPropertyDefinitionBySystemName(sysName);
                    if (propDef != null)
                    {
                        vdfPropDefs.Add(propDef);
                    }
                }
                ...
                {
                    if (propMonikers.TryGetValue(writeReq.Moniker, out string dispName) &&
                        overridePropValues.TryGetValue(dispName, out string overrideValue))
                    {
